...
func NewSSHManifest(appName string, bootstrapUrl string) (manifest *Manifest) {
	manifest = NewManifest()
	cfssh := manifest.AddApplication(appName)
	cfssh["command"] = "curl "+bootstrapUrl+"|sh"
	cfssh["no-route"] = true
	cfssh["instances"] = 1
	return
}
...
func NewSSHManifestFromManifestPath(manifestPath string, bootstrapUrl string) (manifest *Manifest, err error) {
